...
function Profile() {
    ...
    useEffect(() => {
        axios.get(`${host_name}:8080/api/user/${id}`, {
            headers: {
        ...
        axios.get(`${host_name}:8080/api/user/${id}/image`)
    ...
    const handleChange = async (field, value) => {
        const user = userInfo;
        user[field] = value;
        await axios.put(`${host_name}:8080${API_URL}/${user.id}`, user);
        fetchUser();
      };

      const fetchUser = async () => {
        const res = await axios.get(`${host_name}:8080/api/user/${id}`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
    ...
    const uploadImage = (file) => {
        ...
        axios.post(`${host_name}:8080/api/upload/${id}`, formData, {
